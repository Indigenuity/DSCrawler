package agarbagefolder.metaanalysis;
...
public class MetaAnalysisMaster extends UntypedActor {
	...
	public MetaAnalysisMaster(int numWorkers) {
	    ...
	    for (int i = 0; i < this.numWorkers; i++) {
	      ActorRef r = getContext().actorOf(Props.create(MetaAnalysisTool.class));
	      getContext().watch(r);
	...
	public void onReceive(Object work) throws Exception {
		try{
			if(work instanceof SiteWork) {
				...
				ActorRef worker = getContext().actorOf(Props.create(MetaAnalysisTool.class));
